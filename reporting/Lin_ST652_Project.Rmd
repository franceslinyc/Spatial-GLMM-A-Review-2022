---
title: "SGLMM (Spatial Generalized Linear Mixed Model): A Review"
author: "Frances Lin"
date: "June 2022"
output: pdf_document
...
The traditional SGLMM and RHZ model both share very similar approach to that of the automodel. The traditional SGLMM model makes use of synthetic predictors $K$ and $L$, and the RHZ model makes use of $L$. The traditional SGLMM is analogous to the uncentred automodel, and the RHZ model is analogous to the centred automodel. Both the traditional SGLMM and uncentred automodel introduce predictors $K$ and the uncentred autocovariate, respectively, that lead to spatial confounding. Both the RHZ model and centred model are designed to fit only residual structure of the data. 


## 4. Sparse reparameterization of the areal SGLMM

The RHZ model does not allow parsimonious fitting of the residual clustering. The model permits structure of negative spatial dependence (i.e. repulsion) in the random effects because the geometry corresponding to the projection $P^{\bot}$ does not account for the underlying graph $G$. The reparameterized model considers an alternative projection that captures the geometry of the models. Utilizing this geometry also leads to dimension reduction of the random effects naturally. 

### The reparameterized model

The random effects for the reparameterized model i) allow patterns of positive spatial dependence (i.e. attraction) and exclude patterns of repulsion and ii) have dimension much smaller than $n$. 
...
Assume $M$ a matrix that contains the first $q \ll n$ eigenvectors of the Moran operator and further assume positive spatial dependence $\lambda_q > 0$ since it is more realistic to assume that neighboring observation tends to be similar, rather than dissimilar, in practice. 
