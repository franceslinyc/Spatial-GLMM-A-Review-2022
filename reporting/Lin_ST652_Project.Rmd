...
The SGLMM (spatial generalized linear mixed model) is a hierarchical model that introduces spatial dependence through a GMRF (Gaussian Markov random field) (Besag et al., 1991). Although the SGLMM was initially proposed for prediction for count data, it has later been applied to estimation and prediction for other types of data (e.g. binary) and found applications in many fields (e.g. ecology, geology, and forestry). 
...
Assume $M$ a matrix that contains the first $q \ll n$ eigenvectors of the Moran operator and further assume positive spatial dependence $\lambda_q > 0$. 
