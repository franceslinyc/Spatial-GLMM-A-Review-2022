...
(1) The sparse SGLMM not only alleviates spatial confounding but also accounts for the underlying graph while achieving dimension reduction naturally. However, due to time constraint, some of the computational details are not well explored in this project. For example, the choice of operators, the choice of priors, MCMC methods and its variants as well as the `C++` code used in the package that enables efficient computation can be explored further. 
