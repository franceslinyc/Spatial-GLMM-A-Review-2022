...
2. Initialize $\beta$, $\delta_S$, etc. 

3. For $i = 1,...,n\_\text{iteration}$, repeat the following steps: 
    - Draw/update $\beta^{(j+1)} \sim N (\beta^{(j)}, \hat{V})$
    - $\eta^{(j+1)} (= g(\cdot)^{(j+1)}) = X\beta^{(j+1)} + M\delta_S^{(j)}$
    - $\eta^{(j)} (= g(\cdot)^{(j)}) = X\beta^{(j)} + M\delta_S^{(j)}$
    - Construct the log-likelihood for $j+1$ and $j$: $y^T \log(\hat{y}) + {(1 - y)}^T \log(1 - \hat{y}) + \cdots$, where $\hat{y} = g^{-1}(\eta)$ 
    - Compute $\log(\alpha)$ the acceptance "log"-ratio by taking the difference of the log-likelihood (the ratio of the likelihood) 
    - Generate $u \sim Unif(0,1)$: 
      - If $u \leq \alpha$, then accept $j+1$ as the next step and update $\beta^{(j+1)}$ to $\beta^{(j)}$, draw $\delta_S^{(j+1)}$, get $\eta^{(j+1)}$ and $\eta^{(j)}$ but/while keeping $\beta^{(j)}$, compute $\log(\alpha)$, etc.: **If... Else...**
      - Else, stay at the current step $j$. 
      
Note. (1) Nested if-else appears to be involved. (2) $\tau$ is updated in the process too but is omitted here. 
