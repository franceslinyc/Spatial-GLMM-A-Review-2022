...
- An areal mixed model: Metropolis-Hastings algorithm for sampling from the posterior distribution of the field of random effects $W$ when components of $W$ exhibit strong *a posteriori* dependence is slow. Various methods such as MCMC block sampler have been proposed. 
...
- The sparse model: Evaluation of the quadratic form $\delta_S^TQ_S\delta_S$ can be $\mathcal{O}(1)$, which makes the model more suitable for large-scale data sets. 
...
- The `infant` data set and its adjacency matrix matrix $A$ can be accessed from **R** package `ngspatial`, and it is a `data.frame` and `matrix` object. 
